...
def run_json_helper(payload: dict, resolve_only: bool) -> dict:
    if not FAL_MPP_HELPER.exists():
        fail(f"Fal MPP helper not found: {FAL_MPP_HELPER}")
    command = ["node", str(FAL_MPP_HELPER)]
    if resolve_only:
    ...
    try:
        ...
        raise RuntimeError(f"Fal MPP request timed out after {error.timeout} seconds") from error

    if completed.returncode != 0:
        raise RuntimeError((completed.stderr or completed.stdout).strip() or "Fal MPP helper failed")
    stdout = completed.stdout.strip()
    if not stdout:
        raise RuntimeError("Fal MPP helper returned no output")
    try:
        return json.loads(stdout)
    except json.JSONDecodeError as error:
        raise RuntimeError(f"Fal MPP helper returned invalid JSON: {stdout[:400]}") from error
...
def generate_asset(source_path: Path, source_root: Path, args) -> dict:
    ...
    out_dir = output_dir_for(relative_path, FAL_MPP_OUTPUT_ROOT)
    if args.dry_run:
        return {
            "status": "dry-run",
    ...
    for attempt in range(1, NETWORK_RETRY_ATTEMPTS + 1):
        try:
            response = run_json_helper(payload, args.resolve_only)
            ...
            if args.resolve_only:
                return {
                    ...
                    "category": category,
                    "provider": "fal-mpp",
                    "model": response.get("model", f"mpp:{args.model}"),
                    "source_path": str(source_path),
            ...
            if not generated:
                raise RuntimeError("Fal MPP returned no generated audio URL")
    ...
    return {
        ...
        "error": str(last_error) if last_error else "Fal MPP request failed",
        "transport": "mpp",
    }


def generate_assets(paths: list[Path], source_root: Path, args) -> list[dict]:
    existing_entries = load_json(FAL_MPP_MANIFEST, [])
    ...
    if args.concurrency <= 1:
        for index, source_path, target in pending:
            ...
            write_json(FAL_MPP_MANIFEST, results)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--generate", action="store_true", help="Generate replacement audio through Fal MPP.")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Build prompts and manifest entries without calling MPP.")
    parser.add_argument("--resolve-only", action="store_true", help="Resolve MPP payer/model without spending or generating media.")
    parser.add_argument("--model", default=DEFAULT_MODEL, help=f"Fal model through Tempo MPP. Default: {DEFAULT_MODEL}")
...
def main() -> None:
    ...
    if args.generate:
        ...
        print(f"[audio-redo] generated_ok={ok_count} generated_errors={error_count} manifest={FAL_MPP_MANIFEST}", flush=True)
