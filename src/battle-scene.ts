...
import { hydrateLocalUserInfo, updateUserInfo } from "#app/account";
...
import { GENERATED_ROSTER_DEXES, isGeneratedRosterSpeciesId } from "#constants/generated-roster";
...
import { maybeStartArchiveLab } from "./debug/archive-lab";
import { maybeStartBattleLab } from "./debug/battle-lab";
import { maybeStartEvolutionLab } from "./debug/evolution-lab";
import { maybeStartItemLab } from "./debug/item-lab";
import { maybeStartSpriteLab } from "./debug/sprite-lab";
...
export class BattleScene extends SceneBase {
  ...
  /** Debug preview escape hatch toggled by Evolution Lab only. */
  public debugAllowUnrosteredPlayerPokemon = false;
  ...
  launchBattle() {
    ...
    // Initialize UI-related aspects and bootstrap the local profile before title.
    this.ui = new UI();
    this.uiContainer.add(this.ui);
    this.ui.setup();

    void maybeStartBattleLab(this).then(startedBattleLab => {
      if (startedBattleLab) {
        return;
      }
      void maybeStartArchiveLab(this).then(startedArchiveLab => {
        if (startedArchiveLab) {
          return;
        }
        void maybeStartEvolutionLab(this).then(startedEvolutionLab => {
          if (startedEvolutionLab) {
            return;
          }
          void maybeStartItemLab(this).then(startedItemLab => {
            if (startedItemLab) {
              return;
            }
            void maybeStartSpriteLab(this).then(startedSpriteLab => {
              if (startedSpriteLab) {
                return;
              }
              void this.bootstrapTitleProfile().finally(() => {
                this.phaseManager.toTitleScreen(false);
  ...
  addPlayerPokemon(
    ...
    const allowUnrosteredEvolution =
      this.debugAllowUnrosteredPlayerPokemon
      || (dataSource instanceof PlayerPokemon && dataSource.allowUnrosteredEvolution);

    if (!allowUnrosteredEvolution && !isGeneratedRosterSpeciesId(species.speciesId)) {
      console.warn(
        `Blocked player species ${SpeciesId[species.speciesId] ?? species.speciesId} outside generated roster; using generated fallback.`,
      );
      species = getPokemonSpecies(GENERATED_ROSTER_DEXES[0]);
      formIndex = undefined;
    ...
    pokemon.allowUnrosteredEvolution = allowUnrosteredEvolution;
  ...
  addEnemyPokemon(
    ...
    } else if (!isGeneratedRosterSpeciesId(species.speciesId)) {
      console.warn(
        `Blocked enemy species ${SpeciesId[species.speciesId] ?? species.speciesId} outside generated roster; rerolling within dex cap.`,
      );
      species = this.randomSpecies(this.currentBattle.waveIndex, level, false, candidate =>
        isGeneratedRosterSpeciesId(candidate.speciesId),
      );
      boss = this.getEncounterBossSegments(this.currentBattle.waveIndex, level, species) > 1;
    }
  ...
  randomSpecies(
    ...
    const rosterSpecies = allSpecies.filter(s => s.isCatchable() && isGeneratedRosterSpeciesId(s.speciesId));
    ...
    const filteredSpecies = speciesFilter
      ? [
          ...new Set(
            rosterSpecies.filter(speciesFilter).map(s => {
      ...
      : rosterSpecies;
    return randSeedItem(filteredSpecies.length > 0 ? filteredSpecies : rosterSpecies);
  }
  ...
  private isWaveMysteryEncounter(battleType: BattleType, waveIndex: number): boolean {
    if (this.gameMode.hasMysteryEncounters && getDailyMysteryEncounter(waveIndex) != null) {
      return true;
    }
