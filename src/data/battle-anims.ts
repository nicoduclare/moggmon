      ...
      onRepeat: () => {
        ...
        for (const frame of spriteFrames) {
          if (frame.target !== AnimFrameTarget.GRAPHIC) {
            ...
            if (g === sprites.length) {
              const newSprite: Phaser.GameObjects.Sprite = globalScene.addFieldSprite(0, 0, anim!.graphic, 0); // TODO: is the bang correct?
      ...
      onRepeat: () => {
        ...
        for (const frame of spriteFrames) {
          ...
          if (graphicFrameCount === sprites.length) {
            const newSprite: Phaser.GameObjects.Sprite = globalScene.addFieldSprite(0, 0, anim!.graphic, 0);
            sprites.push(newSprite);
            globalScene.field.add(newSprite);
          }
