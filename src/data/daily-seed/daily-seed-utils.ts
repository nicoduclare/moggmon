...
export function validateDailyStarterConfig(config: DailySeedStarter): DailySeedStarter | null {
  ...
  if (!isGeneratedRosterSpeciesId(config.speciesId)) {
    console.warn("Blocked custom daily run seed starter outside generated roster:", config.speciesId);
    return null;
  }
...
export function validateDailyBossConfig(config: DailySeedBoss): DailySeedBoss | null {
  ...
  if (!isGeneratedRosterSpeciesId(config.speciesId)) {
    console.warn("Blocked custom daily run seed boss outside generated roster:", config.speciesId);
    return null;
  }
