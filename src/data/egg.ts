...
export class Egg {
  ...
  constructor(eggOptions?: IEggOptions) {
    const generateEggProperties = (eggOptions?: IEggOptions) => {
      ...
      this._species =
        eggOptions?.species && isGeneratedRosterSpeciesId(eggOptions.species)
          ? eggOptions.species
          : this.rollSpecies()!; // TODO: Is this bang correct?
  ...
  private rollSpecies(): SpeciesId | null {
    ...
    let speciesPool = Object.keys(speciesEggTiers)
      ...
      .filter(
        s =>
          isGeneratedRosterSpeciesId(s) && !pokemonPrevolutions.hasOwnProperty(s) && ignoredSpecies.indexOf(s) === -1,
      );
...
export function getValidLegendaryGachaSpecies(): SpeciesId[] {
  return Object.entries(speciesEggTiers)
    ...
    .filter(s => isGeneratedRosterSpeciesId(s) && s !== SpeciesId.ETERNATUS);
}
