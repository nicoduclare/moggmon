...
export class Arena {
  ...
  public randomSpecies(waveIndex: number, level: number, attempt = 0, luckValue = 0, isBoss?: boolean): PokemonSpecies {
    ...
    if (overrideSpecies && isGeneratedRosterSpeciesId(overrideSpecies.speciesId)) {
    ...
    let tierPool = this.pokemonPool[tier].filter(isGeneratedRosterSpeciesId);
    while (tierPool.length === 0 && tier > BiomePoolTier.COMMON) {
      console.log(`Downgraded capped species pool from ${BiomePoolTier[tier]} to ${BiomePoolTier[tier - 1]}`);
      tier--;
      tierPool = this.pokemonPool[tier].filter(isGeneratedRosterSpeciesId);
    }
    ...
    if (newSpeciesId !== species.speciesId) {
      if (!isGeneratedRosterSpeciesId(newSpeciesId)) {
        console.warn(
          `Blocked level-based species replacement ${SpeciesId[newSpeciesId] ?? newSpeciesId} outside generated roster.`,
        );
        return species;
      }
