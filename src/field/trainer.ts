...
export class Trainer extends Phaser.GameObjects.Container {
  ...
  genPartyMember(index: number): EnemyPokemon {
    ...
    globalScene.executeWithSeedOffset(
      () => {
        ...
        newSpeciesPool = newSpeciesPool.filter(isGeneratedRosterSpeciesId);
        if (newSpeciesPool.length === 0) {
          useNewSpeciesPool = false;
        }
        ...
        if (!isGeneratedRosterSpeciesId(species.speciesId)) {
  ...
  genNewPartyMemberSpecies(level: number, strength: PartyMemberStrength, attempt?: number): PokemonSpecies {
    ...
    if (this.config.speciesPools) {
      ...
      let tierPool = this.config.speciesPools[tier].flat().filter(isGeneratedRosterSpeciesId);
      while (tierPool.length === 0 && tier > TrainerPoolTier.COMMON) {
        console.log(
          `Downgraded trainer Pokemon rarity tier from ${TrainerPoolTier[tier]} to ${TrainerPoolTier[tier - 1]}`,
        );
        tier--;
        tierPool = this.config.speciesPools[tier].flat().filter(isGeneratedRosterSpeciesId);
      }
      if (tierPool.length === 0) {
        return globalScene.randomSpecies(battle.waveIndex, level, false, this.config.speciesFilter);
      }
      const rolledSpecies = randSeedItem(tierPool);
