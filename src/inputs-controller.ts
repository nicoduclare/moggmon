...
export class InputsController {
  ...
  setupKeyboard(): void {
    for (const layout of ["default"]) {
      const config = deepCopy(this.getConfigKeyboard(layout)) as CustomKeyboardConfig;
      config.custom = (this.configs[layout] as CustomKeyboardConfig | undefined)?.custom || { ...config.default };
      config.custom.KEY_SHIFT = -1;
