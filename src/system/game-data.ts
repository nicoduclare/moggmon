...
import { clientSessionId, isLocalProfileMode, loggedInUser, updateUserInfo } from "#app/account";
...
export class GameData {
  ...
  public saveSystem(): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      ...
      const useLocalData = isLocalProfileMode();
  ...
  public loadSystem(): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      ...
      if (useLocalData && !localStorage.getItem(`data_${loggedInUser?.username}`)) {
        return resolve(true);
      }
  ...
  public tryExportData(dataType: GameDataType, slotId = 0): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      ...
      if (!isLocalProfileMode() && dataType < GameDataType.SETTINGS) {
