...
export class PokeballTray extends Phaser.GameObjects.Container {
  ...
  private getBaseX(): number {
    return this.player ? globalScene.scaledCanvas.width : 0;
  }

  private getBaseY(): number {
    return this.player ? -72 : -144;
  }

  private getBallBaseX(index: number): number {
    return (
      (this.player ? -83 : 76)
      + globalScene.scaledCanvas.width * (this.player ? -1 : 1)
      + 10 * index * (this.player ? 1 : -1)
    );
  }

  private updateBallFrames(party: Pokemon[]): void {
  ...
  reset(): void {
    globalScene.tweens.killTweensOf([this, this.bg, ...this.balls]);
    this.setPosition(this.getBaseX(), this.getBaseY());
    this.bg.width = 104;
    this.bg.alpha = 1;
    this.balls.forEach((ball, b) => {
      ball.setPosition(this.getBallBaseX(b), -8);
      ball.setFrame("empty");
    });
    this.setVisible(false);
    this.shown = false;
