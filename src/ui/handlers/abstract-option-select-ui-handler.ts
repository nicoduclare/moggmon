...
export abstract class AbstractOptionSelectUiHandler extends UiHandler {
  ...
  protected optionSelectHitAreas: Phaser.GameObjects.Zone[];
  ...
  protected setupOptions() {
    ...
    this.destroyOptionSelectHitAreas();
    ...
    this.setupPointerHitAreas();
  }

  private destroyOptionSelectHitAreas(): void {
    if (this.optionSelectHitAreas?.length === 0) {
      return;
    }

    this.optionSelectHitAreas.forEach(hitArea => hitArea.destroy());
    this.optionSelectHitAreas.splice(0, this.optionSelectHitAreas.length);
  }

  private getVisibleOptionEntries(): Array<{ fullCursor: number; option: OptionSelectItem } | null> {
    const options = this.config?.options ?? [];
    const maxOptions = this.config?.maxOptions;

    if (!maxOptions || options.length <= maxOptions) {
      return options.map((option, optionIndex) => {
        const fullCursor = this.unskippedIndices.indexOf(optionIndex);
        return fullCursor === -1 ? null : { fullCursor, option };
      });
    }
    ...
    const optionEndIndex = Math.min(
      options.length,
      optionStartIndex
        + (!optionStartIndex || this.scrollCursor + (maxOptions - 1) >= options.length
          ? maxOptions - 1
          : maxOptions - 2),
    );

    const entries: Array<{ fullCursor: number; option: OptionSelectItem } | null> = options
      .slice(optionStartIndex, optionEndIndex)
      .map((option, index) => {
        const optionIndex = optionStartIndex + index;
        const fullCursor = this.unskippedIndices.indexOf(optionIndex);
        return fullCursor === -1 ? null : { fullCursor, option };
      });
    ...
    if (optionEndIndex < options.length) {
      entries.push(null);
    }
  ...
  private setupPointerHitAreas(): void {
    if (!this.config) {
      return;
    }

    const rowHeight = 114 * this.scale - 3;
    const rowTop = this.optionSelectTextContainer.y - rowHeight * 0.25;
    const rowLeft = this.optionSelectBg.x - this.optionSelectBg.width;
    const entries = this.getVisibleOptionEntries();
    ...
    entries.forEach((entry, rowIndex) => {
      ...
      const hitArea = globalScene.add
        .zone(rowLeft, rowTop + rowIndex * rowHeight, this.optionSelectBg.width, rowHeight)
        .setOrigin(0, 0)
        .setInteractive({ useHandCursor: true });

      hitArea.on("pointerover", () => {
        this.setMouseCursorStyle("pointer");
        if (this.setCursor(entry.fullCursor) && this.config?.supportHover) {
          entry.option.onHover?.();
        }
      });
      ...
      hitArea.on("pointerdown", () => {
        this.setCursor(entry.fullCursor);
        this.processInput(Button.ACTION);
      });

      this.optionSelectContainer.add(hitArea);
      if (this.cursorObj) {
        this.optionSelectContainer.bringToTop(this.cursorObj);
      }
      this.optionSelectHitAreas.push(hitArea);
    });
  }
