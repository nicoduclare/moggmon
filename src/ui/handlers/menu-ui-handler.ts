...
import { canLogOutCurrentProfile, hydrateLocalUserInfo, loggedInUser, updateUserInfo } from "#app/account";
import { clearStoredAuraSession } from "#app/aura-login";
...
const wikiUrl = "https://moggermon.gg/docs";
const discordUrl = "https://moggermon.gg/community";
const githubUrl = "https://github.com/auramaxx/moggmon";
const redditUrl = "https://moggermon.gg/community";
const donateUrl = "https://moggermon.gg";
...
export class MenuUiHandler extends MessageUiHandler {
  ...
  constructor(mode: UiMode | null = null) {
    ...
    this.excludedMenus = () => [
      ...
      { condition: !canLogOutCurrentProfile(), options: [MenuOptions.LOG_OUT] },
    ];
        ...
        case MenuOptions.LOG_OUT: {
          ...
          const doLogout = () => {
            ui.setMode(UiMode.LOADING, {
              buttonActions: [],
              fadeOut: async () => {
                if (loggedInUser?.profileSource === "aura") {
                  clearStoredAuraSession();
                  await hydrateLocalUserInfo();
                } else {
                  await Promise.resolve(bypassLogin ? null : pokerogueApi.account.logout());
