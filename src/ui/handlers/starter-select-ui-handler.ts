...
type StarterSelectCancelCallback = () => void;

function formatArchiveId(speciesId: number): string {
  return `AR-${padInt(Math.max(getDexNumber(speciesId), 0), 4)}`;
}
...
export class StarterSelectUiHandler extends MessageUiHandler {
  ...
  */
  ...
  private starterSelectCancelCallback: StarterSelectCancelCallback | null;
  ...
  setup() {
    ...
    const bgColor = globalScene.add.rectangle(0, 0, sWidth, sHeight, 0x282c3b).setOrigin(0);
    ...
    this.pokemonNumberText = addTextObject(6, 1, formatArchiveId(0), TextStyle.SUMMARY_DEX_NUM).setOrigin(0);
    ...
    for (const species of allSpecies) {
      ...
      if (!GENERATED_STARTER_SPECIES_SET.has(species.speciesId)) {
        continue;
      }
  ...
  show(args: any[]): boolean {
    ...
    if (args.length > 0 && args[0] instanceof Function) {
      ...
      this.starterSelectCancelCallback = args[1] instanceof Function ? (args[1] as StarterSelectCancelCallback) : null;
  ...
  setSpecies(species: PokemonSpecies | null) {
    ...
    if (species && (this.speciesStarterDexEntry?.seenAttr || this.speciesStarterDexEntry?.caughtAttr)) {
      this.pokemonNumberText.setText(formatArchiveId(species.speciesId));
      ...
      this.pokemonNumberText.setText(formatArchiveId(0));
  ...
  tryExit(): void {
    ...
    ui.showText(i18next.t("starterSelectUiHandler:confirmExit"), null, () => {
      ui.setModeWithoutClear(
        ...
        () => {
          ui.setMode(UiMode.STARTER_SELECT);
          const originalStarterSelectCancelCallback = this.starterSelectCancelCallback;
          this.starterSelectCancelCallback = null;
          if (originalStarterSelectCancelCallback) {
            this.clearText();
            originalStarterSelectCancelCallback();
          } else {
