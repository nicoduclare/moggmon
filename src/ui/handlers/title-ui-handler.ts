...
export class TitleUiHandler extends OptionSelectUiHandler {
  ...
  private getUsername(): string {
    ...
    const displayName = globalScene.hideUsername
      ? usernameReplacement
      : (loggedInUser?.displayName ?? loggedInUser?.username ?? i18next.t("common:guest"));
  ...
  setup() {
    ...
    this.titleContainer.add([logo, this.usernameLabel, this.playerCountLabel, this.appVersionText]);
  }
  ...
  show(args: any[]): boolean {
    ...
    if (!this.playerCountLabel.visible) {
      this.usernameLabel.setY(UPPER_LABEL);
    } else if (this.usernameLabel.width < this.playerCountLabel.width) {
      this.usernameLabel.setY(UPPER_LABEL);
